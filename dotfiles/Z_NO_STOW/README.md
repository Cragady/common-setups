...
* If something is in a subdir, or subdirs, that is not named `ZZZZ_FLYBYS/`, `ZZZZ_REFS`, or `ZZZZ_PARTIALS/` in the first level of this dir, it's for organizational purposes only
...
* `--` means a dash - `-`, and for every n dashes where n > 2, the dash count is n - 1
...
* `ZZZZ_REFS` contains things for reference, or older configs that no longer have use. The nomenclature in this dir may or may not follow the rules previously outlined
